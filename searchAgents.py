        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                if (nextx, nexty) in state[1]:
                    templist = list(state[1]).copy()
                    templist.remove((nextx, nexty))
                    unvisited_corner_sets  = tuple(templist)
                    successors.append((((nextx, nexty), unvisited_corner_sets), action, 1))
                else: 
